...
export class SalarySetupComponent {
  ...
  getSalarySetUpList() {
      ...
      next: (response: any) => {
        ...
        if (status === true) {
          ...
          // }));
           this.PayArr =[]
           this.DedArr=[]
         this.basic=response.data.basics.map((item: any) => ({ ...item, component_type: "payable" }))
          this.SalArr = [
   ...
   this.PayArr = response.data.allowances.map((item: any) => ({ ...item, component_type: "payable" }))
     this.DedArr=  response.data.deductions.map((item: any) => ({ ...item, component_type: "deductible" }))
